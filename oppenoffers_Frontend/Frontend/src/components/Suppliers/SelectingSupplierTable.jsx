      ...
      <table className="w-full border-collapse border border-gray-300 text-sm">
        ...
        <tbody>
          {filteredSuppliers.map((supplier) => {
            const assignedOps = supplier.OperationIdRetrait 
              ? supplier.OperationIdRetrait.split(',') 
              : [];
            
            const alreadyAssigned = assignedOps.includes(selectedOperationForSupplier);
            ...
            return (
              <tr key={supplier.Id} className="hover:bg-gray-50">
                ...
                <td className="border border-gray-300 px-4 py-2 text-center">
                  {alreadyAssigned ? (
                    <span className="text-sm text-green-600 font-medium italic">Déjà assigné</span>
                  ) : (
                    <button
